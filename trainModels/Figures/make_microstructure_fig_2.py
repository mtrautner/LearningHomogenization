...
fig, ax = plt.subplots(4,6, figsize = (60,35))
cbarpad = 0.02
...
for j in range(4):
    ...
    im1 = ax[j,0].imshow(get_chi1(all_chi_true[j]), cmap = cmap, vmin = ymin, vmax = ymax)
    im2 = ax[j,1].imshow(get_chi1(all_chi_pred[j]), cmap = cmap, vmin = ymin, vmax = ymax)
    # Add colorbars, make them the same limits
    
    cbar1 = fig.colorbar(im1, ax = ax[j,0], fraction=0.046, pad=cbarpad)
    cbar2 = fig.colorbar(im2, ax = ax[j,1], fraction=0.046, pad=cbarpad)
   
    
    

    im3 = ax[j,2].imshow(get_chi1(all_chi_error[j]), cmap = cmap)
    cbar3 = fig.colorbar(im3, ax = ax[j,2], fraction=0.046, pad=cbarpad)
    ...
    im4 = ax[j,3].imshow(get_grad_chi1(all_grad_chi_true[j]), cmap = cmap, vmin = ymin, vmax = ymax)
    im5 = ax[j,4].imshow(get_grad_chi1(all_grad_chi_pred[j]), cmap = cmap, vmin = ymin, vmax = ymax)
    # Add colorbars, make them the same limits
    cbar4 = fig.colorbar(im4, ax = ax[j,3], fraction=0.046, pad=cbarpad)
    cbar5 = fig.colorbar(im5, ax = ax[j,4], fraction=0.046, pad=cbarpad)

    im6 = ax[j,5].imshow(get_grad_chi1(all_grad_chi_error[j]), cmap = cmap)
    cbar6 = fig.colorbar(im6, ax = ax[j,5], fraction=0.046, pad=cbarpad)
    ...
    for k in range(6):
        ax[j,k].set_xticks([])
        ax[j,k].set_yticks([])
        ax[j,k].text(-0.15, 1.02, '('+letters[6*j+k]+')', transform=ax[j,k].transAxes, 
...
ax[0,0].set_title(r'True $\chi_1$', fontsize = fontsize)
ax[0,1].set_title(r'FNO $\chi_1$', fontsize = fontsize)
ax[0,2].set_title(r'Error $\chi_1$', fontsize = fontsize)
ax[0,3].set_title(r'True $\nabla \chi_1$', fontsize = fontsize)
ax[0,4].set_title(r'FNO $\nabla \chi_1$', fontsize = fontsize)
ax[0,5].set_title(r'Error $\nabla \chi_1$', fontsize = fontsize)
...
ax[0].set_title('Smooth', fontsize = fontsize)
ax[1].set_title('Star', fontsize = fontsize)
ax[2].set_title('Square', fontsize = fontsize)
ax[3].set_title('Voronoi', fontsize = fontsize)
plt.tight_layout()
plt.savefig('microstructure_example_A_only.pdf')
