...
for j in range(4):
    ...
    # Add colorbar
    cbar0 = fig.colorbar(im0, ax = ax[j,0], fraction=0.046, pad=0.04)
    ...
    cbar1 = fig.colorbar(im1, ax = ax[j,1], fraction=0.046, pad=0.04)
    cbar2 = fig.colorbar(im2, ax = ax[j,2], fraction=0.046, pad=0.04)
   
    
    

    im3 = ax[j,3].imshow(get_chi1(all_chi_error[j]), cmap = cmap)
    cbar3 = fig.colorbar(im3, ax = ax[j,3], fraction=0.046, pad=0.04)
    ...
    # Add colorbars, make them the same limits
    cbar4 = fig.colorbar(im4, ax = ax[j,4], fraction=0.046, pad=0.04)
    cbar5 = fig.colorbar(im5, ax = ax[j,5], fraction=0.046, pad=0.04)

    im6 = ax[j,6].imshow(get_grad_chi1(all_grad_chi_error[j]), cmap = cmap)
    cbar6 = fig.colorbar(im6, ax = ax[j,6], fraction=0.046, pad=0.04)
